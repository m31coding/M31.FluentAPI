...
namespace PersonExample2
{
    /* Let's create the builder methods 'WhoseAddressIsUnknown', 'WhoLivesAtAddress' and 'WhoIsADigitalNomad' to handle
       ...
       'WhoseAddressIsUnknown', the fluent API should stop, which can be conveniently achieved by adding the attribute
       FluentBreak.
       When 'WhoLivesAtAddress' is called, the API continues with steps 2, 3, and 4, setting the three
       address properties HouseNumber, Street, and City, respectively. I have indented the respective builder methods
...
namespace PersonExample3
{
    /* The attribute FluentContinueWith indicates a jump to the specified builder step. In this example, when
       ...
       API stops, since there are no further steps.
       With the introduction of step 5, the API no longer stops after the 'InCity' method at step 4. To address this,
       I've added the FluentBreak attribute to the 'InCity' method. */
...
{
    /* Lastly, this example demonstrates how the FluentContinueWith attribute can be used to create optional builder
